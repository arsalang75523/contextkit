...
function summarizeTask(mode?: string) {
  ...
  if (mode === "compact") return `${base} Generate only compact and state. compact must be concise, grounded, and shorter than extended: structured state snapshot, no filler status, no inferred themes, no generic business jargon. Do not generate micro, extended, summary, metrics, token counts, debug fields, keyDecisions, actionItems, openQuestions, or risks.`;
