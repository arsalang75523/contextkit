...
function summarizeTask(mode?: string) {
  ...
  return `${base} Generate only micro and state. micro must be materially shorter than compact, usually 18-28 tokens, using terse fragments. Preserve explicit worldview/constraints/anti-patterns before status wording. Never output generic topic labels, filler status, broken fragments, or invented continuation themes. Do not generate compact, extended, summary, metrics, token counts, debug fields, keyDecisions, actionItems, openQuestions, or risks.`;
}
