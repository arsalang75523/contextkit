...
function summarizeTask(mode?: string) {
  ...
  return `${base} Generate only micro and state. micro must be the smallest useful strategic state capsule, max 40 tokens, using terse fragments if useful. It must not be a generic topic label and must not invent continuation themes. Do not generate compact, extended, summary, metrics, token counts, debug fields, keyDecisions, actionItems, openQuestions, or risks.`;
}
