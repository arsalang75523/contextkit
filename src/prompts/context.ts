...
const taskInstructions: Record<ContextEndpoint, string> = {
  summarize:
    "Create a state-first context reduction, not a report. Extract canonical state first, then render micro, compact, and extended from that state. state.goal MUST come from the conversation goal/objective/purpose; if the input says phrases like 'the goal is', 'goal:', 'objective', 'aim', 'trying to', or 'need to', extract that as state.goal. Use 'unknown' for state.goal only when no goal, objective, purpose, or requested outcome is present anywhere in the conversation. micro is a minimum-token strategic continuation state, not a topic label: preserve primary goal plus only explicit continuation-critical constraints, operational worldview, anti-patterns/forbidden directions, reasoning frame, and next direction. Use dense state-vector fragments; remove status filler such as 'work active', 'awaiting execution', or 'core capabilities in place'; avoid unsupported themes, examples, and prose. compact is a faithful mid-level state snapshot, not an interpretive summary: preserve only explicit goal, factual status, hard constraints/blockers, durable decisions, and next actions. Do not add abstraction layers such as enterprise readiness, system resilience, API reliability, operational maturity, or primary risk unless those exact ideas are present. No filler phrases, no why-rationale, no narrative glue, no repeated constraints. extended is a human-readable compressed project continuation format: one paragraph, 80-150 preferred and 200 tokens max, preserving goal, current state, major completed work, up to 5 future-impacting architecture decisions, current focus, and immediate next actions without chronological replay or excessive rationale. Remove duplication across state fields. Do not invent openQuestions or risks; return empty arrays unless explicit evidence exists.",
  "compress-context":
...
function summarizeTask(mode?: string) {
  ...
  if (mode === "compact") return `${base} Generate only compact and state. state.goal is canonical: compact MUST NOT repeat the goal, goal label, or an equivalent goal clause already present in state.goal. compact must be concise, grounded, and shorter than extended: structured state snapshot, no filler status, no inferred themes, no generic business jargon. Do not generate micro, extended, summary, metrics, token counts, debug fields, keyDecisions, actionItems, openQuestions, or risks.`;
  return `${base} Generate only micro and state. micro must be materially shorter than compact, usually 18-28 tokens and never above 35. Optimize information per token: prefer highest-risk blocker, deployment state, unresolved dependency, and critical next action over broad mission wording. Use dense operational nouns and terse fragments. Avoid generic phrases like reduce incidents, improve visibility, core capabilities in place, or goal is to. Never output broken fragments, repeated goal clauses, filler status, or invented continuation themes. Do not generate compact, extended, summary, metrics, token counts, debug fields, keyDecisions, actionItems, openQuestions, or risks.`;
}
