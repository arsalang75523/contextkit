...
const taskInstructions: Record<ContextEndpoint, string> = {
  summarize:
    "Create a state-first context reduction, not a report. Extract canonical state first, then render micro, compact, and extended from that state. state.goal MUST be the durable product, project, or user outcome; never use a transient task such as testing credentials, validating callbacks, fixing a bug, or implementing an integration as the goal when a broader end-state exists. If the input says phrases like 'the goal is', 'goal:', 'objective', 'aim', 'trying to', or 'need to', extract that as state.goal. Use 'unknown' only when no durable objective or requested outcome exists anywhere in the conversation. state.blockers must contain blocking conditions or unresolved dependencies, such as missing credentials, unvalidated environments, capacity limits, or approvals; NEVER put imperative tasks (test, implement, configure, fix, validate) in blockers. Put those imperative tasks in state.nextSteps. Put explicit unverified technical work, blocked validation, and missing dependencies in state.blockers; do not leave blockers empty when the source says verification/confirmation is still required. micro is a minimum-token strategic continuation state, not a topic label: preserve primary goal plus only explicit continuation-critical constraints, operational worldview, anti-patterns/forbidden directions, reasoning frame, and next direction. Use dense state-vector fragments; remove status filler such as 'work active', 'awaiting execution', or 'core capabilities in place'; avoid unsupported themes, examples, and prose. compact is a faithful mid-level state snapshot, not an interpretive summary: preserve only explicit goal, factual status, hard constraints/blockers, durable decisions, and next actions. Do not add abstraction layers such as enterprise readiness, system resilience, API reliability, operational maturity, or primary risk unless those exact ideas are present. No filler phrases, no why-rationale, no narrative glue, no repeated constraints, and no sentence fragments. extended is a human-readable compressed project continuation format: one paragraph, 80-150 preferred and 200 tokens max, preserving goal, current state, explicit blockers, major completed work, up to 5 future-impacting architecture decisions, current focus, and immediate next actions without chronological replay or excessive rationale. Remove duplication across state fields. Do not invent openQuestions or risks; return empty arrays unless explicit evidence exists.",
  "compress-context":
