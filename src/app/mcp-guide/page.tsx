    ...
    <main className="relative min-h-screen overflow-hidden px-5 py-8 md:py-10">
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <section className="mt-16">
          ...
          <div className="mt-8 grid gap-px overflow-hidden rounded-[1.3rem] border border-line bg-line sm:grid-cols-2 lg:grid-cols-3">{["contextkit_summarize", "contextkit_compress_context", "contextkit_handoff", "contextkit_extract_profile", "contextkit_estimate_tokens", "contextkit_get_credits"].map((tool, index) => <div key={tool} className="flex items-center gap-3 bg-carbon/90 p-4"><span className="font-mono text-[10px] text-white/35">0{index + 1}</span><code className="text-sm text-mint">{tool}</code></div>)}</div>
        </section>
