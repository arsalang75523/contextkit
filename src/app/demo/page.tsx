    ...
    <main className="relative min-h-screen overflow-hidden px-5 py-8 md:py-10">
      ...
      <div className="relative mx-auto max-w-7xl">
        <section className="overflow-hidden rounded-[1.55rem] border border-white/[0.13] bg-carbon/80 shadow-[0_24px_90px_rgba(0,0,0,0.3)] backdrop-blur-xl">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-5 py-3 font-mono text-[10px] uppercase tracking-[0.16em] text-white/42 sm:px-7"><span className="flex items-center gap-2"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-mint" /> Demo pipeline / live</span><span className="hidden sm:inline">one request, five continuity outputs</span><span className="text-mint">core endpoint ready</span></div>
          <div className="grid gap-7 px-6 py-8 sm:px-9 lg:grid-cols-[1.08fr_0.92fr] lg:px-12 lg:py-10">
            <div><div className="inline-flex items-center gap-2 rounded-full border border-mint/25 bg-mint/[0.07] px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-mint"><Sparkles className="h-3.5 w-3.5" /> Full pipeline demo</div><h1 className="mt-5 text-balance text-4xl font-semibold leading-[0.98] tracking-[-0.05em] text-white sm:text-5xl">Watch one conversation become reusable agent state.</h1><p className="mt-4 max-w-2xl leading-7 text-white/60">Run real summarize, compression, handoff, profile, and memory extraction in one request. Public Bankr traffic now routes these core modes through <code>contextkit-core</code>.</p></div>
            <div className="grid content-start gap-px overflow-hidden rounded-2xl border border-line bg-line"><Info icon={<BrainCircuit className="h-4 w-4" />} title="Read conversation" text="Plain agent messages become structured input." /><Info icon={<Workflow className="h-4 w-4" />} title="Run core modes" text="Summary, compression, handoff, profile, memory via contextkit-core." /><Info icon={<FileOutput className="h-4 w-4" />} title="Inspect real output" text="Tokens, latency, cost, and continuation payloads." /></div>
          </div>
        </section>
